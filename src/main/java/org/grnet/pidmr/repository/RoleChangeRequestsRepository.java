...
public class RoleChangeRequestsRepository implements Repository<RoleChangeRequest, Long>{
    ...
    /**
     * Finds role change requests by user ID with pagination.
     *
     * @param userId the ID of the user whose role change requests are to be retrieved
     * @param page The index of the page to retrieve (starting from 0).
     * @param size The maximum number of Providers to include in a page.
     * @return a paginated list of role change requests
     */
    public Pageable<RoleChangeRequest> fetchRoleChangeRequestByUser(int page, int size, String userId){

        var panache = find("from RoleChangeRequest r where r.userId = ?1", Sort.by("status", Sort.Direction.Ascending).and("requestedOn", Sort.Direction.Descending), userId).page(page, size);
        var pageable = new PageableImpl<RoleChangeRequest>();
        pageable.list = panache.list();
        pageable.index = page;
        pageable.size = size;
